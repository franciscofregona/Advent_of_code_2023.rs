    ...
    for line in lines.lines() {
        if parsing_movements == false {
            ...
            match parse_move(line) {
                Some((mut qty, origin, destination)) => {
                    match ship.get(&origin) {
                        Some(origin_v) => match ship.get(&destination) {
                            Some(destination_v) => {
                                let destination_v = destination_v.to_owned();
                                let mut origin_v = origin_v.to_owned();

                                // guard on empty origin vector
                                if origin_v.is_empty() {
                                    continue;
                                }

                                // fix the qty to the size of the origin vector
                                if origin_v.len() < qty {
                                    qty = origin_v.len();
                                }

                                let u: Vec<_> = origin_v.drain(0..qty).collect();
                                let destination_v: Vec<char> = [u, destination_v].concat();
